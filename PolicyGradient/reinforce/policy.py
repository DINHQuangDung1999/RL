...
class Softmax_Policy(nn.Module):
    ...
    '''
    def __init__(self, num_inputs, hidden_size, dim_action_space):

        super(Softmax_Policy, self).__init__()

        self.linear1 = nn.Linear(num_inputs, hidden_size)
        self.linear2 = nn.Linear(hidden_size, dim_action_space)
...
class Gaussian_Policy(nn.Module):
    ...
    def __init__(self, num_inputs, hidden_size, dim_action_space):
        ...
        self.mean = nn.Linear(hidden_size, dim_action_space)
        self.log_std = nn.Linear(hidden_size, dim_action_space)
